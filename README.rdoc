...
It uses the Ripper class found in ruby 1.9.2 (and later) to parse and indent the source code.  As a consquence only ruby 1.9.2 (or later)
syntax is parsed correctly.

== TODO

Optimisation. Parses and fontifies whole buffer for most modifications - it still appears to run fast enough on large files.
